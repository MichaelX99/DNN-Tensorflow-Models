...
def inference(images):
    ...
    rescale1 = helper.convolution(pool1, [1, 1, 64, 256], helper.DECAY, 2, 'rescale1')
    ...
    # conv4_B
    conv4_B = helper.bottleneck_convolution(conv3_B, 64, [3,3,256,256], helper.DECAY, 2, 'conv4_B') + conv3_B

    # projection1
    proj1 = helper.projection(conv3_B, conv4_B, helper.DECAY, 'proj1')

    # conv5_B
    conv5_B = helper.bottleneck_convolution(proj1, 128, [3,3,256,512], helper.DECAY, 1, 'conv5_B') + proj1
    ...
    # conv8_B
    conv8_B = helper.bottleneck_convolution(conv7_B, 128, [3,3,512,512], helper.DECAY, 2, 'conv8_B') + conv7_B
    ...
    # conv9_B
    conv9_B = helper.bottleneck_convolution(proj2, 256, [3,3,512,1024], helper.DECAY, 1, 'conv9_B') + proj2
    ...
    # conv14_B
    conv14_B = helper.bottleneck_convolution(conv13_B, 256, [3,3,1024,1024], helper.DECAY, 2, 'conv14_B') + conv13_B

    # projection3
    proj3 = helper.projection(conv13_B, conv14_B, helper.DECAY, 'proj3')

    # conv15_B
    conv15_B = helper.bottleneck_convolution(proj3, 512, [3,3,1024,2048], helper.DECAY, 1, 'conv15_B') + proj3

    # conv16_B
    conv16_B = helper.bottleneck_convolution(conv15_B, 512, [3,3,2048,2048], helper.DECAY, 1, 'conv16_B') + conv15_B

    # conv17_B
    conv17_B = helper.bottleneck_convolution(conv16_B, 512, [3,3,2048,2048], helper.DECAY, 2, 'conv17_B') + conv16_B
